...
// Get the friend's username or user ID from the query string
$friendUserId = isset($_GET['user']) ? $_GET['user'] : null;

// Validate the friendUserId to ensure it's not empty
if (!$friendUserId) {
    die('Invalid user parameter.'); // Or redirect to an error page
}
// Fetch the friend's username
$friendUserName = $friendUserId ? friendName($friendUserId) : 'Unknown';

// Fetch shared albums for the user's friends
$sharedAlbums = getSharedAlbumsByFriend($friendUserId, $userId);
...
<div class="container" style="margin-bottom: 120px;">
    <h2 class="mt-3"><span style="color: DarkSlateBlue;"><?php echo htmlspecialchars($friendUserName); ?></span>'s Pictures </h2>
    <!-- Dropdown menu -->
    <select class="form-select mb-3" id="albumSelect" name="albumSelect" onchange="fetchPhotosByAlbum(this.value)">
        ...
        <option value="<?php echo $album['Album_Id']; ?>" <?php echo $albumId == $album['Album_Id'] ? 'selected' : ''; ?>>
